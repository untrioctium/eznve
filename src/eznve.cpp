...
eznve::encoder::encoder(uint2 dims, uint2 fps, codec c, CUcontext ctx, data_callback_t&& cb) : dims(dims), data_cb(std::move(cb)) {
	...
	auto config_package = std::make_unique<config_package_t>();
...
public:
	output_bitstream_mapper(void* session, void* out_stream, NV_ENC_LOCK_BITSTREAM* mapped) : session(session), out_stream(out_stream) {
		...
		info = eznve::chunk_info{
			.data = std::span<const char>{ (const char*)mapped->bitstreamBufferPtr, mapped->bitstreamSizeInBytes},
...
private:
	eznve::chunk_info info;
	void* out_stream = nullptr;
	void* session = nullptr;
};

bool eznve::encoder::submit_frame(frame_flag flag) {
	const auto& funcs = api().funcs();
	...
	if (frame_status == NV_ENC_ERR_NEED_MORE_INPUT) return false;
